//Modified from Wavelet Filter form Institut Pasteur.
...
public class MicroWaveletFilter {
	...
	public MicroWaveletFilter(ImagePlus imp, boolean[] aScalesList, double aScaleThreshold) {
	...
	public float[] filter(ImageProcessor ip) {
		...
		// decompose the image
		float[][] scales = WaveletScales2D(pixels, width, height, maxScale);
		...
		Result = SpotConstruction2D(coefficients, maxScale, width * height, scalesList);
		return Result;
	...
	private float[][] WaveletScales2D(float[] dataIn, int w, int h, int numScales) {
...
/***
	...
	private float[] SpotConstruction2D(float[][] inputCoefficients, int numScales, int numPixels, boolean[] scaleList) {
