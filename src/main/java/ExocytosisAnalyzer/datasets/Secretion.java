...
public class Secretion {
	...
	private double peak_radius = 0;
	private double peak_radius_r2 = 0;
	...
	public double getEstimatedPeakRadius2D() {
		if (peak_radius == 0) {
			peak_radius = secretion_event.elementAt(peakposition).EstimateSize2D();
		}
		return peak_radius;
	...
	public double getPeakGaussfitterRsquare2D() {
		if (peak_radius_r2 == 0) {
			peak_radius_r2 = secretion_event.elementAt(peakposition).get2DFitRsqr();
		}
		return peak_radius_r2;
	}
	...
	public double getMaxDisplacement() {
		int[] trajectory_x = this.getTrajectoryX() ;
		int[] trajectory_y = this.getTrajectoryY() ;
		
		double Distance_2 = 0;
		double max = 0;
		
		for (int i = 1; i < trajectory_x.length; i++) {
			Distance_2 = Math.pow( trajectory_x[i]-trajectory_x[0], 2) +  Math.pow( trajectory_y[i] - trajectory_y[0], 2);
