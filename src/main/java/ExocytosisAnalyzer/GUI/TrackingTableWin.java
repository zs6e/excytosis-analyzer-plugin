...
public class TrackingTableWin extends JFrame {
	...
	public TrackingTableWin(ImagePlus imp, ResultStackWindow preview_win, Vector<Secretion> input_secretions,
		...
		MouseAdapter ResultListMouseAdapter = new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				...
				if (e.getButton() == MouseEvent.BUTTON3) {
					...
					int RoiX = selectedSecretion.peakX - Math.round (parameters.minSize/2)  - 8;
					int RoiY = selectedSecretion.peakY - Math.round (parameters.minSize/2)  - 8;

					output_ips.setRoi(RoiX, RoiY, Math.round (parameters.minSize/2)  + 16, Math.round (parameters.minSize/2)  + 16);
				}
		...
		ListSelectionListener ResultListSelectionListener = new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				...
				if (secretionEventList.getSelectedRow() != -1
					...
					int RoiX = s.getStartX() - Math.round (parameters.minSize/2)  - 8;
					int RoiY = s.getStartY() - Math.round (parameters.minSize/2)  - 8;
					output_ips.setRoi(RoiX, RoiY, Math.round (parameters.minSize/2)  + 16, Math.round (parameters.minSize/2)  + 16);
		...
		// Save button
		JButton SaveButton = new JButton("Save Event List");
		SaveButton.addActionListener((ActionEvent e) -> {
			try {

				SaveUtils.saveTrackingList(detected_secretions, parameters);
		...
		JMenuItem saveSecretionList = new JMenuItem("Save Event List");
		...
		JMenuItem saveSelectedEvent = new JMenuItem("Save Selected Event ");
		saveSelectedEvent.addActionListener((ActionEvent e) -> {
			try {
				SaveUtils.saveVesicleList_tracking(selectedSecretion.secretion_event, parameters,
		...
		JMenuItem saveFilms = new JMenuItem("Save Labeled Stack");
		...
		saveFilms.addActionListener((ActionEvent e) -> {
			...
			for (Vesicle v : Vesicles) {
				...
				export.getProcessor().drawOval((int) Math.round(v.x - (Math.round (parameters.minSize/2)  + 8)),
						(int) Math.round(v.y - (Math.round (parameters.minSize/2)  + 8)),
						(int) Math.round((parameters.minSize + 16)),
						(int) Math.round((parameters.minSize + 16)));
			}
		...
		JMenuItem viewCruvePlotM = new JMenuItem("Intensity Curve");
		...
		JMenuItem SortByF0 = new JMenuItem("Sort by F0 ");
		SortByF0.addActionListener((ActionEvent e) -> {
				...
				for (int i = 0; i < detected_secretions.size(); i++) {
					if (currentSlice >= detected_secretions.elementAt(i).getStartSlice()
							...
							&& Math.abs(clickPoint.x - detected_secretions.elementAt(i).getStartX()) <= 5
									+ Math.round (parameters.minSize/2) 
