...
public class ResultTableWin extends JFrame {
	...
	public ResultTableWin(ImagePlus imp, ResultStackWindow preview_win, Vector<Secretion> input_secretions,
		...
		columnTitleExocytosis = new String[] { "Ref", // 1
				"Property", // 2
				"Begin Frame", // 3
				"Duration", // 4
				"Peak Frame", // 5
				...
				"Est. Radius (" + parameters.pixelUnit + ")", // 8
				"R² for Est. Radius", // 9
		...
		columnTitleVesicle = new String[] { "Ref", // 1
				"Property", // 2
				"Frame", // 3
				"Intensity", // 4
				"x", // 5
				"y", // 6
				"Est. Radius (" + parameters.pixelUnit + ")", // 7
				"R²" // 8
		};
		...
		for (Secretion s : detected_secretions) {
			tableData[i] = new Object[] { s.getRef(), s.proprety, s.getStartSlice(), s.getDuration(), s.peakTime,
					s.getStartX() + "," + s.getStartY(), format.format(s.getMaxDisplacement() * parameters.pixelSize),
					format.format(s.getEstimatedPeakRadius2D() * parameters.pixelSize),
					format.format(s.getPeakGaussfitterRsquare2D()),
					format.format(s.Decay_tau * parameters.timeInterval), format.format(s.Decay_R2),
					format.format(s.getSNR()), format.format(s.getMin()), format.format(s.getDeltaF()) };
			i++;
		}
		...
		ListSelectionListener ResultListSelectionListener = new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				if (secretionEventList.getSelectedRow() != -1
					...
					int RoiX = s.peakX - parameters.minRadius - 8;
					int RoiY = s.peakY - parameters.minRadius - 8;
					output_ips.setRoi(RoiX, RoiY, parameters.minRadius + 16, parameters.minRadius + 16);
		...
		JButton viewRadiusFitterButton = new JButton("Radius");
		viewRadiusFitterButton.addActionListener((ActionEvent e) -> {
			Secretion s = detected_secretions.elementAt(selectedTableRow);
			Vesicle v = s.secretion_event.elementAt(s.peakTime - s.getStartSlice());
			viewVesicleRadiusFitter(v);
		});
		viewRadiusFitterButton.setToolTipText("View 2D Gaussien fitter");
		...
		JButton viewRadiusHisBtn = new JButton("Radius Stat.");
		...
		viewCruvePlotM.setToolTipText("View intensity curve and fitter");
		...
		JMenuItem viewRadiusFitterM = new JMenuItem("Radius");
		viewRadiusFitterM.addActionListener((ActionEvent e) -> {
			Secretion s = detected_secretions.elementAt(selectedTableRow);
			Vesicle v = s.secretion_event.elementAt(s.peakTime - s.getStartSlice());
			viewVesicleRadiusFitter(v);
		});
		viewRadiusFitterM.setToolTipText("View the Gaussien fitter");

		JMenuItem viewRingSimplingCruveM = new JMenuItem("Ring Simpling");
		...
		JMenuItem SortByRadius = new JMenuItem("Sort by Radius");
		SortByRadius.addActionListener((ActionEvent e) -> {
			Collections.sort(detected_secretions, new comparator_Radius());
			refreshSecretionList();
		});

		JMenuItem SortByRadiusR2 = new JMenuItem("Sort by R² for Radius ");
		SortByRadius.addActionListener((ActionEvent e) -> {
	...
	private void viewCruvePlot(Secretion aSecretion) {
		...
		intensityPlot
				.addLegend("Fit: y=" + format.format(aSecretion.Decay_bk) + "+" + format.format(aSecretion.Decay_peak)
	...
	private void viewRingSimplingCruvePlot(Secretion aSecretion) {
		...
		for (int i = 1; i < cruveNum; i++) {
			RingSimpling.setColor(new Color(0, (i - 1) * 40, 295 - i * 40));
	...
	private void viewTauHis() {
		...
		his_tau_win.setTitle("Tau distribution");
	...
	private void viewRadiusHis() {
		double[] estimated_radius = new double[detected_secretions.size()];

		for (int j = 0; j < detected_secretions.size(); j++) {
			estimated_radius[j] = detected_secretions.elementAt(j).getEstimatedPeakRadius2D() * parameters.pixelSize;
		}

		Plot his_radius = new Plot("Radius distribution", parameters.pixelUnit, "");
		his_radius.addHistogram(estimated_radius, radiusHisBin);
		PlotWindow his_radius_win = his_radius.show();
		his_radius_win.setTitle("Radius distribution");
		Panel settingPanel = new Panel();
		Button changeBTN = new Button("Change");
		Label Binlabel = new Label("binWidth :");
		Label Binlabel2 = new Label(" (set zero for auto-binning)");
	...
	private void saveCurvePlot(Secretion aSecretion, String path, String name) {
		...
		intensityPlot.addLegend(
				"Fitter: y=" + format.format(aSecretion.Decay_bk) + "+" + format.format(aSecretion.Decay_peak) + "*"
						+ "exp(-x/" + format.format(aSecretion.Decay_tau * parameters.timeInterval) + ")");
	...
	public class comparator_Radius implements Comparator<Object> {
		public int compare(Object o1, Object o2) {
			Secretion s1 = (Secretion) o1;
			Secretion s2 = (Secretion) o2;
			if (s1.getEstimatedPeakRadius2D() > s2.getEstimatedPeakRadius2D())
				return -1; // decrease
			else if (s1.getEstimatedPeakRadius2D() < s2.getEstimatedPeakRadius2D())
				return 1;
			else
				return 0;
		}
	...
	private class EventWin extends JFrame {
		...
		private static final long serialVersionUID = 1L;
		private ResultTable selectedEventTable;
		...
		EventWin(Secretion selected_secretion) {
			...
			selectedEventTable = new ResultTable(secretion_dtm);
