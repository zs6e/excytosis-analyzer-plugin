...
public class MicroWaveletDetector {
	...
	public MicroWaveletDetector(ImageProcessor ip, boolean[] aScalesList, double aScaleThreshold){
		input =ip ;
	...
	public float[] filter() {
		...
		int minSize = 5+(int)(Math.pow(2, maxScale-1)-1)*4;//compute the minimum size for maxScales
		if (width < minSize || height < minSize) return Result;
		
		final float[] pixels = (float[]) input.convertToFloatProcessor().getPixels();
		//decompose the image
	...
	private float[][]  b3WaveletScales2D(float[] dataIn, int w, int h, int numScales) {
		...
		//store wavelet scales in a new 2d  array
		...
		for (int s = 1; s <= numScales; s++) {
			...
			if (s==1){
				...
				currentArray = new float[w*h]; //allocate memory for the next dimension filtering (preserve original data)
			}
			else{
	...
	private void filter_wat( float[] data, int depth, int numPixels, int numScales, boolean[] aScalesList, double aScaleThreshold){
		...
		double dcoeff[] = new double[ numScales  ]; 
		for ( int i = 0 ; i < numScales  ; i++ ){
			dcoeff[i] = aScaleThreshold / 100.0d;
		}
	 ...
	 */
	private float[] b3SpotConstruction2D(float[][] inputCoefficients, int numScales, int numPixels, boolean[] scaleList){
